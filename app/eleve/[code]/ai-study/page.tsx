...
export default function StudentAIStudyPage() {
  ...
  const generate = async () => {
    if (!student?.level || !subject) return;
  ...
  return (
    <main className="ai-student">
      ...
      <div className="ai-student-wrap">
        ...
        <div className="level-box">🎓 Niveau : {student?.level || '-'}</div>
        <div className="student-badge">🔐 Ton niveau et tes matières viennent directement de ton espace élève.</div>
        <div className="tools">
        ...
        <section className="card">
          <div className="grid">
            <div className="field"><label>Matière</label><select value={subject} onChange={e => setSubject(e.target.value)}><option value="">Choisir une matière</option>{subjects.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}</select></div>
          ...
          <button className="generate" disabled={generating || !student?.level || !subject} onClick={generate}>{generating ? 'Génération...' : tool === 'tutor' ? '🤖 Demander au Tutor' : tool === 'flashcards' ? '📚 Générer mes Flashcards' : '🧪 Générer mon Practice Test'}</button>
